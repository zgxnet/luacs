...
namespace Lua.Internal
{
    ...
    internal sealed class LuaValueDictionary
    {
        ...
        public LuaValue this[string key] //zgx 20250625
        {
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            get
            {
            ...
            if (_buckets != null)
            {
                Debug.Assert(_entries != null, "expected entries to be != null");
                {
                    ...
                    do
                    {
                        ...
                        entry = ref entries[i];
                        if (entry.hashCode == hashCode && entry.key.IsString && entry.key.UnsafeAsString() == key)
                        {
                            index = i;
                            goto ReturnFound;
                        }
        ...
        private void Insert(string key, in LuaValue value) //todo: merge with up insert
        {
            if (value.type is LuaValueType.Nil)
            {
                _nilCount++;
